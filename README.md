...
游標在模擬面板輸入框時，以上快捷鍵不作用。導覽列「儀表板」直接跳到第 3 頁。

## 調整模擬數值：`src/data/config.json`

百分比欄位一律以「%」為單位（`70` 代表 70%）。修改後先執行 `npm test`，再重新 `npm run build`；地圖、摘要卡、TOP 10、模擬判定、第 2 頁規則卡與附錄都會同步更新。

`npm test` 會驗證 config 是否仍符合敘事約束。若設定彼此矛盾（例如全市削減率無法達成），資料產生器會拋出「config.json 設定錯誤：…」並說明原因。注意：測試中寫死了目前的預期值（4 個未達標區、68.5% 等），刻意修改這些值後，對應的測試也要一起更新。

| 欄位 | 說明 |
|---|---|
| `year` | 年度（115） |
| `seed` | 亂數種子。改變它會重新洗牌工地數、工程類型、一般工地排放與削減率，但下列所有約束仍成立 |
| `moenvTargets` | 環境部年度目標，如 `{ "115": 65, "116": 67 }` |
| `zones.<metro/developing/rural>` | 分區設定：`label` 名稱、`code` 工地代號字母、`target` 分區目標、`densityRule` 顯示用門檻文字、`minDensity` 門檻下限（附錄門檻線）、`siteCountRange` 施工中工地數範圍 |
| `districts[]` | 29 區：`name`、`density` 人口密度、`zone` 所屬分區、`manualAdjusted` 是否手動調整、`adjustReason` 調整原因（手動調整時必填，附錄會顯示） |
| `underperformingDistricts` | **未達標行政區與其削減率**，如 `{ "新莊": 66.8, ... }`。必須低於該區分區目標 |
| `compliantMarginRange` | 其餘達標區的削減率範圍 = 分區目標 + `[min, max]`（預設 `[1, 10]`） |
| `cityReductionRate` | **全市削減率**（排放量加權，預設 68.5） |
...
| `estimatedTriggerCases` | 摘要卡「本年度預估觸發案件數」（預設 7） |
...
工程類型可用值：`建築（RC/SRC）`、`道路／隧道`、`捷運／軌道`、`區段徵收／重劃`、`管線`、`橋梁／水利`（僅「建築（RC/SRC）」屬房屋建築工程）。
